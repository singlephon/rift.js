...
export default class RiftV1 {
    ...
    start () {
        this.evaluate(this.RiftHooks.beforeInit);
            ...
            Livewire.hook('commit', ({ component, commit, respond, succeed, fail }) => {
                succeed(({ snapshot, effect }) => {
                    ...
                    if (component._x_dataStack)
                        component._x_dataStack[0].rift._rift_synchronizer_(snapshot.data);
                })
            })

            this.evaluate(this.RiftHooks.afterInit);
        });
    }
    ...
    evaluate (fn, type = null, errorHandler = null) {
        if (type && !this.isTypeof(fn, 'function'))
            return;
